...
export class WaitingCustomersComponent implements OnInit {
  ...
  AddInTempCustomerTable() {
    ...
    console.log("Customer Status ", this.obserCustomer.obIcustomer.status);
    console.log("Temp Status ", this.TempService.temp.status);

    this.TempService.temp.salesID = this.obserCustomer.obIcustomer.salesRepId;

    console.log("Customer salesID ", this.obserCustomer.obIcustomer.salesRepId);
    console.log("Temp salesID ", this.TempService.temp.salesID);
    ...
    console.log("Temp Actual Customer ID ", this.TempService.temp.actualCustomerId);
  ...
  Setgrid() {
    this.columnDefs = [
      {
        headerName: this.Rtl ? 'كود العميل' : 'Customer Id', field: 'actualID', width: 150,
