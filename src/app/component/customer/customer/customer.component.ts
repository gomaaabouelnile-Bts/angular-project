...
export class CustomerComponent implements OnInit {
    ...
    if (this.Admin == false) {
      ...
      this.SerSalesRep.GetByUserID(this.LoginedUserIDCustomer).subscribe(
        res => {
          ...
          if (res != 0) {
            this.rowData = this.obserCustomer.getCustomersBySalesRep(this.LoginedSalesRepID);
          }
        }
      );
      this.Status = 0;
  ...
  onSubmit(f: NgForm) {
    ...
    this.obserCustomer.obIcustomer.status = this.Status;
    this.obserCustomer.obIcustomer.originalID = this.OriginalID;
