...
export class EvalRepComponent implements OnInit {
  ...
  columnDefs = [
   { headerName: this.Rtl ? 'السؤال' : 'Question Description', field: 'questionName', width: 200, sortable: true, filter: true },
    { headerName: this.Rtl ? 'المتوسط' : 'Average', field: 'average', width: 150, sortable: true, filter: true },
  ];
    ...
    this.CustomerVisitsServ.AvRateProcedure(this.CustID).subscribe(res => {
      this.CustomerVisitsServ.AvPercentageProcedure(this.CustID).subscribe(res2 => {
        res.forEach(element => {
          ...
          this.CustAvItem.sector = element.sector;
          
          this.CustAvItem.region = element.region;
          
          this.CustAvItem.governorate = element.governorate;
          
          this.CustAvItem.territory = element.territory;
          
          this.CustAvItem.company = element.company;
