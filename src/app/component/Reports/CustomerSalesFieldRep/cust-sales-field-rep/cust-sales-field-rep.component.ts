...
export class CustSalesFieldRepComponent implements OnInit {
  ...
  onGridReady(params) {
    ...
    this.CustomerSalesFieldServ.CustomerSalesFieldViews().subscribe(res => {
  ...
  Filter() {
    ...
    if (this.CustID != 0) {
      this.CustomerClassViewList.forEach(element => {
        if (element.salesFieldID == this.CustID) {
      ...
      Copy.forEach(Listelement => {
        if (idelement == Listelement.customerId) {
          if (i != index) {
            ...
            x.customerClassID = Listelement.customerClassID;
          ...
          x.salesFieldName=x.salesFieldName+","+ Listelement.salesFieldName;
