...
UpdateFooterRow(template: TemplateRef<any>)
{
  ...
  {
    this.update = 1;
    this.title = 'Edit';// + this.obserSectorService.oboneSector.sectorLatName ;
...
Setgrid()
{
  ...
  this.columnDefsinsert = [
   ...
   // tslint:disable-next-line: max-line-length
   { headerName: this.Rtl ? 'الوصف العربى' : 'sector Name', field: 'sectorName',
    width: 300 , sortable: true, filter: true, editable : true},
    // tslint:disable-next-line: max-line-length
    {headerName: this.Rtl ? 'الوصف الانجليزى' : 'sector Lat Name', field: 'sectorLatName'
    ,  width: 300 , sortable: true, filter: true, editable : true }
  ];
