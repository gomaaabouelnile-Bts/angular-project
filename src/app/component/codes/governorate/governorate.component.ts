...
export class GovernorateComponent implements OnInit {
  ...
  constructor(public obserGovernorate: GovernorateService, private spinner: NgxSpinnerService
    ,         private translate: TranslateService, private modalService: BsModalService,
              public obserRegionService: RegionService, public obserTerritory: TerritoryService
...
insertFooterRow(template: TemplateRef<any>) {
  ...
  if (items.length > 0) {
  this.obserRegionService.oboneRegion.regionId =
  Math.max.apply(Math, items.map( function(o) { return o.regionId; })) + 1;
  console.log(items);
  console.log(this.obserRegionService.oboneRegion.regionId);
  }
  else {
  this.obserRegionService.oboneRegion.regionId = 1;
  console.log(items);
  }
  this.modalRef = this.modalService.show(template);
}
...
Setgrid()
{
  ...
  this.columnDefsinsert = [
   ...
   // tslint:disable-next-line: max-line-length
   { headerName: this.Rtl ? 'الوصف العربى' : 'Region Name', field: 'regionName',
    width: 300 , sortable: true, filter: true, editable : true},
    // tslint:disable-next-line: max-line-length
    {headerName: this.Rtl ? 'الوصف الانجليزى' : 'Region Lat Name', field: 'regionLatName'
    ,  width: 300 , sortable: true, filter: true, editable : true }
  ];
